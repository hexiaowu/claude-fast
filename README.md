...
> 本目录为**纯源码库**（与 GitHub 仓库一致）。程序本体通过**安装包**分发（Windows：`Claude助手_<版本>_x64-setup.exe`；macOS：`.app` + `Claude助手_<版本>_<架构>.dmg`）；用户数据（项目清单与设置 `config.json`）在安装版数据目录 `%APPDATA%\claude-fast`（macOS 为 `~/Library/Application Support/claude-fast`）。
...
- **列表排序** → 任意项目行**整行拖拽**手动排序（搜索过滤期间禁用拖拽），顺序保存在 `config.json`，重启后依然有效；新发现/未拖拽过的项目按名称排在已排序项目之后
...
- **右键菜单** → 打开所在文件夹、复制路径、从列表移除（从列表移除的项目不会再被会话扫描自动加回）
...
- **config.json 保护**：`save_config` 采用「写临时文件 → 备份旧文件到 `.bak` → 原子替换」三步；`load_config` 读取失败时自动从 `.bak` 回退。**绝不删除 `config.json` / `.bak`**，否则用户的项目清单与设置丢失。
- **旧启动脚本迁移（一次性）**：去脚本化前的版本把项目存在 `scripts/` 下的启动脚本里。旧 config（无 `projects` 字段）首次被新版加载时，解析旧脚本的 `cd` 路径（兼容 bat 的 `cd /d "..."` 与 sh 的 `cd "/path"`）自动迁移为项目清单；脚本文件保留在磁盘不删，可自行清理。
...
- **前端**（`src/`）：`App.tsx` 状态管理 + 组件化 UI（列表 / 搜索 / 右键菜单 / 各对话框），`src/lib/api.ts` 封装 Tauri invoke 调用。
...
- 项目清单、手动排序、主题、自启动等全部保存在 `config.json`；从列表移除的项目会记入排除清单，不会被会话扫描自动加回。
